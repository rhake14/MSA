...
make_practice_page <- function(page_no, audio_dir) {
  psychTestR::reactive_page(function(answer, ...) {
    # browser()
    correct <- "INCORRECT"
    if (page_no > 1) {answer = answer[1,]} # because the item gets loaded 3 times for some reason
...
get_reproducible_seed <- function() {
  ...
  # Calculate minutes since the beginning of the current 10-minute block
  minutes_in_block <- floor(time_in_seconds / 60) %/% 2 # set this to the value you like
...
get_practice_page_long <- function(page_no, feedback, audio_dir) {
 ...
 # include video
  ...
  if (page_no == 5) {
    ...
    sample_data_long <- MSA::MSA_itembank_training_only[1:6,]
